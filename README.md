...
- その上に**ライブラリから Wan 用の LoRA を最大 8 個**まで重ねられます（蒸留 2 本と合わせて API のサニティ上限 10 本）。指定は URL ではなく**名前**（`.safetensors` 抜き）で、Modal Volume に無いものは初回リクエスト時に取り込まれます。この画面から「Hugging Face から一括登録」「Civitai から取り込み」もできます
...
- ライブラリを通さず、**「名前を直接入力…」で LoRA 名を打つこともできます**。この API は URL ではなく名前で LoRA を指定するので、ベースモデルの表記に関係なく使えます（Modal Volume に無い名前は、初回リクエスト時に取り込まれます）
