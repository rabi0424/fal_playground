...
`VAPID_*` は未設定でも構いません（その場合はプッシュ通知の機能だけが無効になります）。旧バージョンで使っていた `SYNC_TOKEN` は不要になったので削除して構いません。
...
- ホーム画面に追加してアプリとして起動（PWA）・生成完了のプッシュ通知（下記）
...
追加したアプリの中でトップバーの **🔕 通知** を押すと、生成の完了をプッシュ通知でお知らせします。

- **通知の中身は「生成が完了しました（3 件）」だけ**です。プロンプト・モデル・LoRA・画像などは一切含めません（ロック画面や通知センターに内容が出ないようにするため）。タップするとアプリが開きます
- **アプリを前面で開いている間は通知しません**。他のアプリに切り替えている間・スリープ（画面ロック）中・アプリを閉じている間だけ届きます
  - 判定は端末ごとに行います。画面から離れた瞬間にブラウザが「見ていない」ことをサーバーへ送り（以後 25 秒ごとのハートビートも止まる）、サーバーはその記録を見て、見ている端末には送信しません。Service Worker 側にも同じ判定があり、取りこぼしても通知は出ません
  - 複数の端末で通知をオンにしている場合、たとえば Mac で生成を始めても、iPhone を伏せていれば iPhone にだけ届きます
- 複数枚の生成やアリーナの一斉生成は、数秒待ち合わせて **1 通にまとめます**（件数付き）。失敗したときも「生成に失敗しました」で通知します
- fal での生成はブラウザがポーリングして完了を検知しているため、アプリを閉じている間はブラウザ側が完了を知れません。そのため送信時に `status_url` をサーバーへ預け、**サーバー側でも完了を見張って**通知します（最大 30 分）。Modal 版 Krea 2 と部分AI編集はもともとサーバー側でジョブが完結するので、そのまま通知されます
...
2. 出力された 2 つの値を Worker の Secret（`VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY`）に設定してデプロイする
3. iPhone で**ホーム画面に追加したアプリから開き**、トップバーの 🔕 通知 を押して許可する

制約・注意:

- **iPhone の Web Push は「ホーム画面に追加したアプリ」の中でしか使えません**（iOS 16.4 以降）。Safari のタブで開いたまま通知をオンにすることはできず、ボタンを押すと追加を促すメッセージが出ます
- 通知のオン/オフは端末ごとです。使う端末それぞれで一度オンにしてください
- 鍵を作り直すと既存の購読は無効になります。各端末で通知をオンにし直してください
- 通知の許可を一度拒否すると、iPhone の「設定 → 通知」から許可し直す必要があります
- 実装は Worker 内で完結しています（`web-push` などのライブラリは使わず、WebCrypto で VAPID の JWT 署名と aes128gcm のペイロード暗号化を行っています）
...
| プッシュ通知の購読情報（端末のエンドポイントと鍵） | サーバー（Durable Object） | 通知をオンにした端末ぶん。オフにする・アプリを削除すると消える（失効を検知した時点でも削除） |
