...
function renderVoteLog(session, round) {
  ...
  for (const m of recent) {
    ...
    a.textContent = participantName(session, m.a);
    if (m.winner === 'a') a.className = 'win';
    const b = document.createElement('span');
    b.textContent = participantName(session, m.b);
...
function renderLeaderboard(session) {
  ...
  order.forEach((p, i) => {
    ...
    name.textContent = loraDisplayName(p.path);
    name.title = p.path;
    tr.appendChild(name);
  ...
  [...session.rounds].reverse().forEach((round) => {
    ...
    if (ok > 0) {
      ...
      for (const p of session.participants) {
        ...
        label.textContent = loraDisplayName(p.path);
        label.title = p.path;
